...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jobDetails || !freelancerDetails) {
      return NextResponse.json(
        { error: 'Failed to retrieve job or freelancer details' },
        { status: 500 }
      )
    }
    ...
    await prisma.notification.create({
      data: {
        userId: jobDetails.clientId,
        title: 'New Job Application',
        message: `${freelancerDetails.name} has applied to your job "${jobDetails.title}"`,
        type: 'JOB_APPLICATION',
        entityId: application.id,
        entityType: 'application',
