...
export default function PostJobForm() {
  ...
  const { user } = useAuth()
  const { requireAuth, isAuthenticated, openLoginDialog, setOpenLoginDialog } = useRequireAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!description.trim()) return setFormError('Job description is required');
    if (!selectedCategory) return setFormError('Service category is required');
    if (!budget.trim()) return setFormError('Budget is required');
    if (!location.trim()) return setFormError('Location is required');
    if (!startDate) return setFormError('Date & time is required');
    ...
    try {
      ...
      // Form is submitted, we can assume auth check passed or handled by API 
      // but double check session if needed, though middleware/API handles it.
      ...
      if (error instanceof Error && error.message.includes('log in')) {
        ...
        setFormError(error instanceof Error ? error.message : 'An error occurred while posting the job')
      }
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-6 pb-32">
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
            <div className="bg-[#111111] border border-white/10 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl scale-100 transform transition-all">
              <div className="mb-6">
                <div className="w-16 h-16 bg-green-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-green-500/20">
                  <CheckCircle2 className="w-8 h-8 text-green-500" />
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Job Posted Successfully!</h3>
                <p className="text-white/60">Your job is now visible to freelancers</p>
              </div>

              <div className="space-y-3">
                <Button
                  onClick={handlePostAnother}
                  className="w-full bg-white text-black hover:bg-gray-200 py-3 font-medium transition-all duration-300 rounded-xl"
                >
                  Post Another Job
                </Button>
                <Button
                  onClick={handleClose}
                  variant="outline"
                  className="w-full border-white/10 bg-white/5 text-white hover:bg-white/10 py-3 font-medium transition-all duration-300 rounded-xl"
                >
                  Close
                </Button>
              </div>
