...
export function DateRangeModal({
  ...
  onSelect,
  onPauseDates = () => { },
  initialStartDate = new Date(2026, 0, 1), // January 1, 2026
  initialEndDate = new Date(2026, 0, 31),   // January 31, 2026
  ...
  useEffect(() => {
    ...
    // Set the minimum start date to January 1, 2026
    const minStartDate = new Date(2026, 0, 1); // Note: months are 0-indexed, so 0 = January

    // Include all 12 months of 2026
    ...
    if (startDate && !isNaN(startDate.getTime())) {
      ...
      // Ensure we don't go before January 2026
    ...
    while (sortedMonths.length < 2) {
      const lastMonth = sortedMonths.length > 0
        ...
        : new Date(2026, 0, 1); // Default to January 2026 if no months exist
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className={`absolute inset-0 w-full h-full flex items-center justify-center ${buttonClasses.filter(Boolean).join(' ')} ${isEnd && !isPast && mode !== 'pause' ? 'group/endDate' : ''
                    }`}
                  style={mode !== 'pause' ? {
